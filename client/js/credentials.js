document.addEventListener('DOMContentLoaded', () => {
    ...
    function FormValidator() {
        ...
        const confirmPassError = clientRegisterform.querySelector("#confirm-pass-error");

        clientRegisterform.addEventListener("submit", function (event) {
            let isValid = true;
    ...
    function submitFormData() {
        ...
        xhr.open("POST", "client-signup.php", true);
    ...
    function displayModal(message) {
        ...
        if (parsedMessage.type === 'error') {
            // Replace success checkmark with error X
            svg.innerHTML = `
                    <path d="M18 6L6 18M6 6l12 12" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                `;
            checkCircle.style.backgroundColor = '#ff3333';
        } else {
            // Reset to success checkmark
            svg.innerHTML = `
    ...
    function clientLogin() {
        clientLoginForm.addEventListener("submit", function (event) {
            ...
            const xhr = new XMLHttpRequest();
            xhr.open("POST", "client-login.php", true);
