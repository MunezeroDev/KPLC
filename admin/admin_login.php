...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $emailCheckStmt = $mysqliObj->prepare("SELECT email, role FROM users WHERE email = ?");
    $emailCheckStmt->bind_param('s', $email);
    $emailCheckStmt->execute();
    $emailCheckStmt->store_result();
    $emailCheckStmt->bind_result($fetched_email, $user_role);
    ...
    $stmt = $mysqliObj->prepare("SELECT email, password_hash, user_id FROM users WHERE email = ? AND password_hash = ? AND role = 'ADMIN'");
...
<body>
    <main>
        <article class="login" aria-labelledby="">
            ...
            <!-- header  -->
            <header class="form-header admin">Admin Login</header>

            <!-- login-form  -->
            <form method="post" id="admin-login-form" class="register-form">
                <!-- email -->
                ...
                <div class="form-group">
                    <div class="password-wrapper">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" class="icon passIcon">
                            <path id="icon-path"
